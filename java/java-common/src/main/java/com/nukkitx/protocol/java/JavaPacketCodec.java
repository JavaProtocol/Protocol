...
import com.nukkitx.protocol.java.exception.PacketSerializeException;
...
public final class JavaPacketCodec {
    ...
    private final JavaPacketSerializer<BedrockPacket>[] serializers;
    ...
    public BedrockPacket tryDecode(ByteBuf buf, int id) throws PacketSerializeException {
        ...
        JavaPacketSerializer<BedrockPacket> serializer;
        try {
            packet = idBiMap.get(id).newInstance();
            if (packet instanceof UnknownPacket) {
                //noinspection unchecked
                serializer = (JavaPacketSerializer<BedrockPacket>) packet;
            } else {
    ...
    public static class Builder {
        private final Int2ObjectMap<JavaPacketSerializer<BedrockPacket>> serializers = new Int2ObjectOpenHashMap<>();
        ...
        public <T extends BedrockPacket> Builder registerPacket(Class<T> packetClass, JavaPacketSerializer<T> serializer, @Nonnegative int id) {
            ...
            serializers.put(id, (JavaPacketSerializer<BedrockPacket>) serializer);
        ...
        public JavaPacketCodec build() {
            ...
            JavaPacketSerializer<BedrockPacket>[] serializers = new JavaPacketSerializer[largestId + 1];

            for (Int2ObjectMap.Entry<JavaPacketSerializer<BedrockPacket>> entry : this.serializers.int2ObjectEntrySet()) {
                serializers[entry.getIntKey()] = entry.getValue();
            }
            return new JavaPacketCodec(protocolVersion, minecraftVersion, serializers, idBiMap, helper, raknetProtocolVersion);
        }
    }
}
